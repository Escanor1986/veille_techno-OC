...
function extractTags(item, defaultTags) {
  ...
  const titleTags = item.title
    ? item.title
        ...
        .filter(word => 
          word.length > 3 && 
          ['test', 'java', 'angular', 'react', 'vue', 'spring', 'junit', 
           'mock', 'api', 'rest', 'web', 'code', 'dev', 'app', 'ui', 'ux']
          .includes(word)
        )
    : [];
  
  // Combiner tous les tags, supprimer les doublons, et limiter à 5 tags
  const allTags = [...new Set([...defaultTags, ...categoryTags, ...titleTags])].slice(0, 5);
...
`;
  ...
  latestArticles.forEach(article => {
    const formattedDate = new Date(article.isoDate).toLocaleDateString('fr-FR', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
...
`;
      ...
      if (data.items.length === 0) {
        ...
        data.items.slice(0, 10).forEach(item => {
          ...
          content += `- [${item.title}](${item.link}) – *${item.pubDate}* ${formatTags(tags)}\n`;
        });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  tags.forEach(tag => {
    ...
    tagFiltersContainer.appendChild(tagBtn);
      ...
      // Sauvegarde des données brutes au format JSON pour utilisation ultérieure
      fs.writeFileSync(
        path.join(dataDir, `${feed.name.toLowerCase().replace(/ /g, '_')}.json`),
